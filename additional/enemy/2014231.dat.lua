...
local class = summoner.Bootstrap.createUnitClass({label="魔獣", version=1.3, id=500021313})
...
function class:hp70(status)
   if status ==  "use3" then
      ...
      -- self.gameUnit:getTeamUnitCondition():addCondition(5213133,0,1,999999,0,50002)
      self:showMessage(self.HP70_MESSAGE_LIST)
      self:execAddBuff(self.gameUnit,self.HP70_BUFF_BOX)
   end
end
...
function class:setMessageBoxList()
   self.START_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "燃焼時被ダメージアップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false        
      },
      [2] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "相手毒時与ダメージアップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false        
      }
   ...
   self.HP50_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP70_MESSAGE1 or "奥義ゲージ速度アップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false        
      }
