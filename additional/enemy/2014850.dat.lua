...
local class = summoner.Bootstrap.createEnemyClass({label="リヴィエラ", version=1.3, id=2014850});
...
function class:setMessageBoxList()
   self.START_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "リヴィエラ",
         COLOR = Color.green,
         DURATION = 5,
         isPlayer = false        
      }
...
function class:hp50(status)
   if status == "use3" then
      self.isRage = true
      self.isFury = true
      self.gameUnit:setInvincibleTime(4)
...
function class:update(event)
   ...
   self:checkAliveUnit(event.unit,250)
   return 1
end

function class:checkAliveUnit(unit,unitBaseID)
   if self.isAliveSetis then
      for i = 0, 7 do
         local localUnit = unit:getTeam():getTeamUnit(i)
         if localUnit ~= nil and localUnit:getBaseID3() == unitBaseID then
            return true
         end
      end
      self.isAliveSetis = false
...
function class:takeDamageValue(event)
   return self.isAliveSetis and 0 or event.value
end
...
function class:skillReroll(unit)
   ...
   if self.isRage and not self.isAura or self.isFury then
      skillIndex = 3
   end
