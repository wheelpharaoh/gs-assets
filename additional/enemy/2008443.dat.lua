...
local class = summoner.Bootstrap.createEnemyClass({label="ダキ", version=1.3, id=2008443});
...
function class:setMessageBoxList()
  self.START_MESSAGES = {
    [0] = {
      MESSAGE = self.TEXT.START_MESSAGE1 or "クリティカル耐性",
      COLOR = Color.red,
      DURATION = 5
    }
  }

  self.FALL_MEO_MESSAGES = {
    [0] = {
      MESSAGE = self.TEXT.FALL_MEO_MESSAGE1 or "そんな…メオ…",
      COLOR = Color.yellow,
      DURATION = 5
    },
    [1] = {
      MESSAGE = self.TEXT.FALL_MEO_MESSAGE2 or "与ダメージUP",
      COLOR = Color.red,
      DURATION = 5
    },
    [2] = {
      MESSAGE = self.TEXT.FALL_MEO_MESSAGE3 or "光属性耐性DOWN",
      COLOR = Color.yellow,
      DURATION = 5
    }
  }
...
function class:isBuddyStillAlive(unit)    
  for i=0,7 do
    ...
    if target ~= nil and target ~= unit and target:getBaseID3() == self.MEO_ID then
      return true;
    end
  end
  return false;
...
function class:fallMEO(unit)
  self:showMessage(self.FALL_MEO_MESSAGES)
