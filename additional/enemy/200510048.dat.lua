...
function class:takeSkill(event)
    ...
    if self:getIsBoss(event.unit) and event.index == 2 then
        for i=0,7 do
            ...
            if target ~= nil and target ~= event.unit and target:getBaseID3() == 182 and megast.Battle:getInstance():isHost() then
...
function class:isBuddyStillAlive(unit)
    ...
    for i=0,7 do
        ...
        if target ~= nil and target ~= unit and target:getBaseID3() == 182 then
            return true;
        end
    end
    return false;
