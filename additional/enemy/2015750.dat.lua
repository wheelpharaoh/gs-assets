...
local class = summoner.Bootstrap.createEnemyClass({label="ラキ", version=1.3, id=2015750});
...
function class:setMessageBoxList()
   ...
   self.HP80_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP80_MESSAGE1 or "ラキさん！援護するね！",
         COLOR = Color.cyan,
         DURATION = 5,
         isPlayer = false
      },
      [2] = {
         MESSAGE = self.TEXT.HP80_MESSAGE2 or "ラキの防御力アップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false
      }
   ...
   self.HP40_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP40_MESSAGE1 or "ラキさん頑張って〜！",
         COLOR = Color.cyan,
         DURATION = 5,
         isPlayer = false
      },
      [2] = {
         MESSAGE = self.TEXT.HP40_MESSAGE2 or "ラキの奥義ゲージ速度アップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false
      }
   ...
   self.HP20_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP20_MESSAGE1 or "ラキさん負けちゃダメだよ〜！",
         COLOR = Color.cyan,
         DURATION = 5,
         isPlayer = false
      },
      [2] = {
         MESSAGE = self.TEXT.HP20_MESSAGE2 or "ラキにHP自然回復効果",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false
      }
...
function class:setTriggerList()
   self.TRIGGERS = {
      ...
      [7] = {
         tag = "OTHER",
         action = function (status) self:stageClear(status) end,
         used = false
      }
...
function class:stageClear(status)
   if status == "use3" then
...
function class:startWave(event)
   self.adelUnit = self:findAdel(event.unit)
   return 1
end

function class:findAdel(unit)
   for i = 0,7 do
      local localUnit = unit:getTeam():getTeamUnit(i)
      if localUnit ~= nil and localUnit:getBaseID3() == 291 then
         -- unit:takeHeal(100)
         return localUnit
      end
   end
...
--    if self.adelUnit ~= nil then self.adelUnit:setHP(0) end
--    self:execTrigger(7)
--    return 1
-- end
