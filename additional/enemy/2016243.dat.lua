...
local class = summoner.Bootstrap.createUnitClass({label="狐タマエ", version=1.8, id=2016243})
...
function class:setMessageBoxList()
   self.START_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "ブレイクで攻撃属性切替　炎⇔闇",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false        
      },
...
function class:takeBreake(event)
   ...
   self:removeConditionAll(self.gameUnit,93967200)
    self:removeConditionAll(self.gameUnit,93967300)
   return 1
end
