...
local class = summoner.Bootstrap.createEnemyClass({label="属性回廊氷巨人", version=1.3, id=2004536});
...
function class:start(event)
    ...
    self.RAGE_MESSAGES1 = {
        [0] = {
            MESSAGE = self.TEXT.RAGE_MESSAGE1 or "HP自然回復",
            COLOR = Color.green,
            DURATION = 15
        },
        [1] = {
            MESSAGE = self.TEXT.RAGE_MESSAGE2 or "味方の水属性耐性ダウン",
            COLOR = Color.cyan,
            DURATION = 15,
            PLAYER_SIDE = true
        }
    ...
    self.START_MESSAGES = {
        [0] = {
            MESSAGE = self.TEXT.START_MESSAGE1 or "毒状態以外ダメージ無効",
            COLOR = Color.red,
            DURATION = 15
        },
        [1] = {
            MESSAGE = self.TEXT.START_MESSAGE2 or "命中率アップ",
            COLOR = Color.yellow,
            DURATION = 15
        },
        [2] = {
            MESSAGE = self.TEXT.START_MESSAGE3 or "暗闇状態で被ダメージアップ",
            COLOR = Color.red,
            DURATION = 15
        },
        [3] = {
            MESSAGE = self.TEXT.START_MESSAGE4 or "人・機族は気絶状態",
            COLOR = Color.red,
            DURATION = 15
        }
    }
...
function class:update(event)
    ...
    event.unit:setReduceHitStop(2,0.5);--ヒットストップ無効Lv2　５割軽減
