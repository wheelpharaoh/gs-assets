...
local class = summoner.Bootstrap.createEnemyClass({label="イフリート", version=1.3, id=2007943});
...
self.MESSAGE_BOX_LIST = {
  ...
  [1] = {
    MESSAGE = self.TEXT.START_MESSAGE2 or "光・闇ダメージ軽減",
    COLOR = Color.yellow,
    DURATION = 5
  },
  [2] = {
...
function class:firstImpact(unit)
  self:showMessageRange(self.MESSAGE_BOX_LIST,0,1)
...
function class:dead(event)
  self:showMessage(self.MESSAGE_BOX_LIST,2)
...
function class:update(event)
  ...
  if event.unit:getTeamUnitCondition():findConditionWithType(97) ~= nil then
      self.isBurn = true
      self.spValue = 40
  else
      self.isBurn = false
