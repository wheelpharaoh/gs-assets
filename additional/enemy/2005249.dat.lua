...
local class = summoner.Bootstrap.createEnemyClass({label="リアナ", version=1.3, id=2005249});
...
function class:setMessageBoxList()
  ...
  self.DEAD_MESSAGE_LIST = {
   [0] = {
      MESSAGE = self.TEXT.DEAD_MESSAGE or "いい腕前ね",
      COLOR = Color.cyan,
      DURATION = 5
    }
  }
...
function class:startWave(event)
    self.arumaUnit = self:buddyUnitCatcher(event.unit,self.ARUMA_ID)
    return 1
end
...
function class:skillBranch(unit)
    ...
    if summoner.Utility.getUnitHealthRate(unit) < 0.5 then
        skillIndex = 3;
        self.arumaUnit:setHP(1)
    end
    self.arumaUnit:addSP(100)
