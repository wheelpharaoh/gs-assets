function new(id)
    ...
    local instance = {
        ...
        messages = summoner.Text:fetchByEnemyID(200310008),
        ...
        addShot = function(this,unit,order)
            local orbit = unit:addOrbitSystem("shot");
            local target = this:findUnitByHPOrder(order);
            orbit:setPosition(target:getPositionX(),target:getPositionY());
            orbit:setDamageRateOffset(1/3);
            orbit:setBreakRate(1/3);
            return 1;
        end,


        findUnitByHPOrder = function(this,order)
                ...
                if target ~= nil then
                    for j=1,5 do
                        ...
                        if units[j] == nil then
                            ...
                            if this.utill.getHPParcent(units[j]) <= this.utill.getHPParcent(target) then
                                local tmp = units[j];
                                units[j] = target;
                                target = tmp;
                            end
        ...
        getRage = function(this,unit)
            ...
            unit:getTeamUnitCondition():addCondition(10157,17,-50,99999,0);
        end,
        ...
        run = function (this , unit , str)
            ...
            if str == "addShot1" then return this.addShot(this,unit,1) end
            if str == "addShot2" then return this.addShot(this,unit,2) end
            if str == "addShot3" then return this.addShot(this,unit,3) end
            return 1;
        end,
        ...
        update = function (this , unit , deltatime)
            if this.isRage and unit:getBreakPoint() > 0 then
            ...
            if this.utill.isHost() then
                if this.utill.getHPParcent(unit) <= 70 and not this.isRage then
