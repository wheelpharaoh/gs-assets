...
local class = summoner.Bootstrap.createEnemyClass({label="2004537", version=1.3, id=2004537});
...
function class:initMessages()
    ...
    self.START_MESSAGES = {
        -- [1] = {
        --     MESSAGE = self.TEXT.START_MESSAGE or "毒状態の相手に対して気絶付与",
        --     COLOR = Color.red,
        --     DURATION = 10
        -- },
        -- [2] = {
        --     MESSAGE = self.TEXT.HP_REDUCE_MESSAGE or "毒状態の相手に対してダメージアップ",
        --     COLOR = Color.red,
        --     DURATION = 10
        -- }
    }
    ...
    self.START_MESSAGE_LIST = {
        [1] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "命中率アップ",
         COLOR = Color.red,
         DURATION = 5,
         isPlayer = false
      }
    ...
    self.HP80_MESSAGE_LIST = {
        [1] = {
         MESSAGE = self.TEXT.HP80_MESSAGE1 or "ブレイク耐性アップ",
         COLOR = Color.red,
         DURATION = 5,
         isPlayer = false
      }
    ...
    self.HP50_MESSAGE_LIST = {
        [1] = {
         MESSAGE = self.TEXT.HP50_MESSAGE1 or "灼熱の大気が吹き荒れる",
         COLOR = Color.red,
         DURATION = 5,
         isPlayer = false
      }
    ...
    self.HP20_MESSAGE_LIST = {
        [1] = {
         MESSAGE = self.TEXT.HP20_MESSAGE1 or "ブレイク耐性アップ",
         COLOR = Color.red,
         DURATION = 5,
         isPlayer = false
      }
...
function class:startWave(event)
    ...
    self:showMessages(self.START_MESSAGE_LIST);
...
function class:countDown(unit,deltaTime)
    ...
    if self.endTimer > 300 and beforTimer <= 300 then
        ...
        self:showMessages(self.RAGE_MESSAGES);
    end
end
...
function class:hp80(status)
   if status == "use3" then
      self:addBuffs(self.gameUnit,self.HP80_BUFF_ARGS);
      self:showMessages(self.HP80_MESSAGE_LIST)
   end
end
...
function class:hp60(status)
   if status == "use3" then
      ...
      self:showMessages(self.HP60_MESSAGE_LIST)
   end
end

function class:hp50(status)
   if status == "use3" then
      self.gameUnit:addSP(self.gameUnit:getNeedSP());
...
function class:hp40(status)
   if status == "use3" then
      self:addBuffs(self.gameUnit,self.HP40_BUFF_ARGS);
      self:showMessages(self.HP40_MESSAGE_LIST)
   end
end

function class:hp20(status)
   if status == "use3" then
      self:addBuffs(self.gameUnit,self.HP20_BUFF_ARGS);
      self:showMessages(self.HP20_MESSAGE_LIST)
   end
end
...
function class:takeDamageValue(event)
   ...
   if event.unit:getTeamUnitCondition():findConditionWithID(self.REFLECTION_BUFFARGS[1].ID) ~= nil then
      local  targetUnit = event.enemy;

      if targetUnit:getParentTeamUnit() ~= nil then
         targetUnit = targetUnit:getParentTeamUnit();
      end
      local reflection = math.floor(event.value * 2);
...
function class:run (event)
    ...
    if event.spineEvent == "skill2Start" then 
      self:skill2Start() 
      self:addBuffs(event.unit,self.REFLECTION_BUFFARGS)
   end
...
-- 1.active状態で生成されます。開幕特殊奥義があるので４へ移行
...
-- 7.start終了後、deactive状態に移行。クリスタルが光っていない待機状態。lightUpされない限りはこの状態です。2のフェーズに戻ります。以降2〜７をループ
