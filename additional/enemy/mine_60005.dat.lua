...
local class = summoner.Bootstrap.createEnemyClass({label="女神", version=1.3, id="mine_60005"});
...
function class:setMessageBoxList()
   self.START_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "防御力アップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false        
      },
...
function class:execAddBuff(unit,buffBox)
   ...
   if not self.isAura then
      self.isAura = true
      buff:addAnimationWithFile("unit/animation/MagicStoneAura.json","aura_Megami"); 
   end
end
