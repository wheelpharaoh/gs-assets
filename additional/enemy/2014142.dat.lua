...
local class = summoner.Bootstrap.createEnemyClass({label="ゴブスレ巨人", version=1.3, id=2014049});
...
function class:start(event)
    ...
    self.START_MESSAGES = {
        [0] = {
            MESSAGE = self.TEXT.START_MESSAGE1 or "GYAGYUGAAA！",
            COLOR = Color.green,
            DURATION = 5
        },
        [1] = {
            MESSAGE = self.TEXT.START_MESSAGE2 or "被ダメージ軽減（ブレイク後解除）",
            COLOR = Color.yellow,
            DURATION = 5
        }
    }
    ...
    self.BREAK_MESSAGES = {
        [0] = {
            MESSAGE = self.TEXT.BREAK_MESSAGE1 or "GOBUUGAGA…！！",
            COLOR = Color.green,
            DURATION = 10
        },
        [1] = {
            MESSAGE = self.TEXT.BREAK_MESSAGE2 or "被ダメージ軽減解除",
            COLOR = Color.yellow,
            DURATION = 10
        }
    }
    ...
    self.RAGE_MESSAGES = {
        [0] = {
            MESSAGE = self.TEXT.RAGE_MESSAGE1 or "GYAGORRRAAAAA！！",
            COLOR = Color.green,
            DURATION = 10
        },
        [1] = {
...
function class:update(event)
    ...
    event.unit:setReduceHitStop(2,1);--ヒットストップ無効Lv2　9.5割軽減
...
function class:takeBreake(event)
    local buff = event.unit:getTeamUnitCondition():findConditionWithID(self.START_BUFF_ARGS[1].ID);
    if buff ~= nil then
