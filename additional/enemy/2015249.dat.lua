--@additionalEnemy,2015250
local Bootstrap, Color, Random, Utility = summoner.import("Bootstrap", "Color", "Random", "Utility")
local class = summoner.Bootstrap.createEnemyClass({label="キサラギ", version=1.3, id=2015249});
...
function class:setMessageBoxList()
   ...
   self.HP70_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP70_MESSAGE1 or "攻撃力アップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false
      }
   ...
   self.HP50_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP50_MESSAGE1 or "奥義ゲージ速度アップ・攻撃力アップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false
      }     
...
function class:hp60(status)
   if status == "use3" then
      ...
      self.gameUnit:setInvincibleTime(5)
   end
end
...
function class:setAura(unit)
   ...
   self.aura = unit:addOrbitSystemWithFile("Kaigan1","Kaigan_in");
   self.aura:takeAnimation(0,"Kaigan_in",true);
