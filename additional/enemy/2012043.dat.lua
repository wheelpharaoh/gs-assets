...
local class = summoner.Bootstrap.createEnemyClass({label="魔族騎士団イベントボス　ニース", version=1.3, id=2012043});
...
function class:start(event)
    ...
    self.START_MESSAGES = {
        [0] = {
            MESSAGE = self.TEXT.START_MESSAGE1 or "炎属性耐性・キラー",
            COLOR = Color.red,
            DURATION = 5
        },
        [1] = {
            MESSAGE = self.TEXT.START_MESSAGE2 or "自然回復",
            COLOR = Color.green,
            DURATION = 5
        }
    }
    ...
    self.RAGE_MESSAGES = {
        [0] = {
            MESSAGE = self.TEXT.RAGE_MESSAGE1 or "受けてみよ！",
            COLOR = Color.cyan,
            DURATION = 5
        }
    }
    ...
    self.LAST_MESSAGES = {
        [0] = {
            MESSAGE = self.TEXT.LAST_MESSAGE1 or "蒼く天を穿つ",
            COLOR = Color.cyan,
            DURATION = 5
        }
    }
...
function class:takeSkill(event)
    ...
    if self.forceActiveSkill ~= 0 and not self.skillCheckFlg2 then
    ...
    if self.forceActiveSkill ~= 0 then
...
function class:run(event)
   	if event.spineEvent == "shieldOpen" then
	    self:takeShieldBuff(event.unit);
	end
   return 1
end
