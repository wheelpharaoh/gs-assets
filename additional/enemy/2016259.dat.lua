...
local class = summoner.Bootstrap.createUnitClass({label="狐タマエ", version=1.8, id=2016259})
...
function class:setMessageBoxList()
   self.START_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "我が名を讃えよ、妖狐、憂月の名を",
         COLOR = Color.magenta,
         DURATION = 5,
         isPlayer = false        
      },
      [2] = {
         MESSAGE = self.TEXT.START_MESSAGE2 or "ブレイクで攻撃属性切替　炎⇔闇",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false        
      },
      [3] = {
         MESSAGE = self.TEXT.START_MESSAGE3 or "攻撃力・命中率アップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false 
      }
   ...
   self.HP10_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP10_MESSAGE1 or "汝らに破滅を贈ろう",
         COLOR = Color.magenta,
         DURATION = 5,
         isPlayer = false        
      }
...
function class:takeBreake(event)
   ...
   self:removeConditionAll(self.gameUnit,93965200)
    self:removeConditionAll(self.gameUnit,93965300)
   return 1
end
