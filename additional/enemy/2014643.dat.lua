--@additionalEnemy,2014650
local Bootstrap, Color, Random, Utility = summoner.import("Bootstrap", "Color", "Random", "Utility")
local class = summoner.Bootstrap.createEnemyClass({label="プラチナ", version=1.3, id=2014643});
...
function class:setMessageBoxList()
   self.START_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "仮置:プラチナです。",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false        
      }
   ...
   self.HP50_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP50_MESSAGE1 or "加勢する！",
         COLOR = Color.green,
         DURATION = 5,
         isPlayer = false
      },
      [2] = {
         MESSAGE = self.TEXT.HP50_MESSAGE2 or "パロットがいる間、プラチナへのダメージ無効",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false
      }
   ...
   self.HP30_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP30_MESSAGE1 or "裁定の門は開いた",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false
      }
...
function class:update(event)
   ...
   self:checkPallot(event.unit,event.deltaTime)
   return 1
end

function class:checkPallot(unit,deltaTime)

   if self.pace < self.timer then
      self.timer = 0
      if self.isAlivePallot then
         for i = 0, 7 do
            local localUnit = unit:getTeam():getTeamUnit(i)
            if localUnit ~= nil and localUnit:getBaseID3() == 169 then
               return true
            end
         end
         self.isAlivePallot = false
...
function class:takeDamageValue(event)
   if event.value >= self.gameUnit:getHP() then self:destroy(event.unit) end

   return self.isAlivePallot and 0 or event.value
end
