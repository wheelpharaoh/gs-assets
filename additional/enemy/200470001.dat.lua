local class = summoner.Bootstrap.createEnemyClass({label="れうす（仮）", version=1.3, id=200470001});
...
function class:attackBranch(unit)
    ...
    --空を飛んでいる間は咆哮なしで怒りに移行するため、空を飛んでいる時の行動優先
    if not self.isGround then
        attackStr = summoner.Random.sampleWeighted(self.ATTACK_WEIGHTS_SKY);
...
function class:skillBranch(unit)
    ...
    if not self.isGround then
        skillStr = summoner.Random.sampleWeighted(self.SKILL_WEIGHTS_SKY);
    end
...
function class:run (event)
    ...
    if event.spineEvent == "takeoff" then self:takeoff(event.unit) end
    if event.spineEvent == "landing" then self:landing(event.unit) end
...
function class:takeoff(unit)
    self.isGround = false;
    unit:setSetupAnimationName("setUpSky");
    unit:setDefaultMainPositionY(362.3);
end

function class:landing(unit)
    self.isGround = true;
    unit:setSetupAnimationName("");
    unit:setDefaultMainPositionY(184.8);
end
...
function class:getRaidTime()
    return false and RaidControl:get():getCurrentRaidTime() or self.timeCount;
end
...
function class:animationSwitcher(unit,animState)
    if self.state ~= self.STATES.FIRST_RAGE and self.state ~= self.STATES.SECOND_RAGE then
        if animState == self.ANIMATION_STATES.IDLE and not self.isGround then
            unit:setNextAnimationName("zcloneNidle2");
        elseif animState == self.ANIMATION_STATES.DAMAGE and not self.isGround then
            unit:setNextAnimationName("zcloneNdamage2");
        elseif animState == self.ANIMATION_STATES.BACK and not self.isGround then
