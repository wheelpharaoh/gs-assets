...
function class:start(event)
    ...
    self.isDead = false
    self.isHeart = self:countUnit(true) == 2 and true or false
...
function class:run (event)
    ...
    if event.spineEvent == "dead" then 
        self.gameUnit:takeAnimation(0,"out2",true)
    end
    return 1;
end
...
function class:clear()
   if self.isHeart and self:countUnit(false) == 2 then
      local orbit = self.gameUnit:addOrbitSystemWithFile("heart","clear");
      orbit:takeAnimation(0,"clear",false);
   end
   self.gameUnit:takeAnimation(0,"out",false)
   megast.Battle:getInstance():setBattleState(kBattleState_none);
   megast.Battle:getInstance():waveEnd(true);
end

function class:countUnit(isExist)
   local count = 0
   for i = 0, 7 do
      local localUnit = megast.Battle:getInstance():getTeam(true):getTeamUnit(i,isExist)
