...
local class = summoner.Bootstrap.createEnemyClass({label="闇フェン", version=1.3, id=2013649});
...
function class:setMessageBoxList()
   self.START_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "楽しませてもらおう",
         COLOR = Color.magenta,
         DURATION = 5,
         isPlayer = false        
      }
   ...
   self.HP50_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP50_MESSAGE1 or "肩慣らしはこれくらいにしておこうか",
         COLOR = Color.magenta,
         DURATION = 5,
         isPlayer = false        
      }
   ...
   self.HP20_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP20_MESSAGE1 or "私を怒らせた罪は重いぞ…",
         COLOR = Color.magenta,
         DURATION = 5,
         isPlayer = false
      }
...
function class:takeSkill(event)
   ...
   if self.isFury then
      self.isFury = false
      event.unit:setActiveSkill(self.ACTIVE_SKILLS.SKILL4)
   else
