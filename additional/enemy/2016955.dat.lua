...
local class = summoner.Bootstrap.createEnemyClass({label="ロイ　チャレンジクエスト", version=1.3, id=2016955});
...
function class:update(event)
    ...
    if self.countDown then
        self.safeTimer = self.safeTimer + event.deltaTime;
        if self.safeTimer > 4 then
            event.unit:setHP(0);
        end
    end
    if self.isDead then
...
function class:takeDamageValue(event)
    ...
    if result <= 0 and self.safeTimer < 4 then
