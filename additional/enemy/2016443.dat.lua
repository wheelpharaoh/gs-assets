...
local class = summoner.Bootstrap.createUnitClass({label="アルズ", version=1.3, id=2016443});
...
function class:setMessageBoxList()
  ...
  self.START_MESSAGE_LIST = {
   ...
   self.HP20_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.HP20_MESSAGE1 or "格の違いを見せてあげるよ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false
      },
      [2] = {
         MESSAGE = self.TEXT.HP20_MESSAGE2 or "奥義ゲージ速度アップ",
         COLOR = Color.yellow,
         DURATION = 5,
         isPlayer = false
      }
...
function class:takeSkill(event)
   ...
   self.fromHost = false;
   if self.isFury then
      event.unit:setActiveSkill(self.TOKUSYU)
      self.isFury = false;
   else
