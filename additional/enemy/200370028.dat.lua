local child = summoner.Bootstrap.createEnemyClass({label="レグルス", version=1.3, id=200370028});
...
function child:takeSkill(event)
    ...
    if self:getIsBoss(event.unit) then
        for i=0,7 do
            ...
            if target ~= nil and target ~= event.unit and target:getBaseID3() == 4 and megast.Battle:getInstance():isHost() then
                target:callLuaMethod("forceSkill",1.5);
            end
        end
    end
    return 1
...
function child:getRage(unit)
    summoner.Utility.messageByEnemy(self.messages.mess1,5,summoner.Color.magenta,36);
    summoner.Utility.messageByEnemy(self.messages.mess2,5,summoner.Color.magenta,26);
