...
local class = summoner.Bootstrap.createEnemyClass({label="セティス", version=1.3, id=2014849});
...
function class:setMessageBoxList()
   self.START_MESSAGE_LIST = {
      [1] = {
         MESSAGE = self.TEXT.START_MESSAGE2 or "リヴィエラ、防御はボクが！",
         COLOR = Color.cyan,
         DURATION = 6,
         isPlayer = false        
      },
      [2] = {
         MESSAGE = self.TEXT.START_MESSAGE3 or "んじゃ、始めよっか！",
         COLOR = Color.green,
         DURATION = 7,
         isPlayer = false        
      },
      [3] = {
         MESSAGE = self.TEXT.START_MESSAGE4 or "セティスがいる間、リヴィエラへのダメージ無効",
         COLOR = Color.yellow,
         DURATION = 8,
         isPlayer = false        
      },
      [4] = {
         MESSAGE = self.TEXT.START_MESSAGE1 or "250秒以内に騎士団全員を倒せ！",
         COLOR = Color.yellow,
         DURATION = 9,
         isPlayer = false        
      }
...
function class:setupSkill3(unit)
   if not self.isAura then
      ...
      unit:setNextAnimationEffectName("skill3b")
   end
end
