...
function initPlayerTeam(team)
    ...
    skill_timer = 1.0;
    skill3_timer = 5;
    local unit = megast.Battle:getInstance():getPlayerTeam():addTeamUnit(101066411,90,4);
...
function zeioSkill()
    print("*****************zeio takeSkill3");
    local zeio = megast.Battle:getInstance():getPlayerTeam():getTeamUnit(4);
    if zeio ~= nil then
        zeio:takeSkill(1);
    end
    return 1;
...
function quest_update(wave,delta,time)
    ...
    if skill3_timer <= 0 then
        ...
        if zeio ~= nil then
            zeio:takeSkill(3);
            zeio:setBurstState(kBurstState_active);
...
function useSkill(index, isBurst)
    if skill_timer < 0 then
        BattleControl:get():callLuaMethod(2000324 , "zeioSkill" , 0.7);
        skill_timer = 6;
    end
    return 1;
end
