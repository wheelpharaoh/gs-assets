...
function initEnemyTeam(team, wave)
    print("initEnemyTeam");
        megast.Battle:getInstance():getTeam(false):removeUnit(6);
    return 1;
end
...
function quest_update(wave , delta ,time)
    if wave < megast.Battle:getInstance():getWaveMax() then
        if  BattleControl:get():getTime() > timelimit and megast.Battle:getInstance():getBattleState() == kBattleState_active then
            ...
            BattleControl:get():callLuaMethod(1001008 , "deadend" , 4);
        end  
    end
    return 1;
end
...
function deadend()
            ...
            BattleControl:get():callLuaMethod(1001008 , "questend" , 0.1);
    return 1;
end
