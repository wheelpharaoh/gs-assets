...
function new(id)
    ...
    -- 所持アイテムリスト
    instance.inventory = { index = 0, id = 100691400 }
    -- 遷移状態と結びつくキーの管理テーブル
    instance.keyNames = { attack1 = "attack1", attack2 = "attack2", attack3 = "attack3", skill1 = "skill1", skill2 = "skill2" }
    -- 遷移状態管理テーブル
    instance.transitions = { attack1 = false, attack2 = false, attack3 = false, skill1 = false, skill2 = false }
    -- アクション名管理テーブル
    instance.actions = { cast = "cast", attack = "attack", skill = "skill" }
    -- 通常攻撃のindex管理テーブル
    instance.attackIndexes = { attack1 = 1, attack2 = 2, attack3 = 3 }
    -- skillのindex管理テーブル
    instance.skillIndexes = { skill1 = 1, skill2 = 2 }
    -- 行動ごとの発生確率管理テーブル
    instance.behaviorProbs = { { value = "attack", prob = 40 }, { value = "cast", prob = 30 }, { value = "skill", prob = 30 } }
    -- 津城攻撃ごとの発生確率管理テーブル
    instance.attackProbs = { { key = "attack1", prob = 30 }, { key = "attack2", prob = 30 }, { key = "attack3", prob = 30 } }
    -- HPのボーダーライン。これを下回ると該当アクションを行うようになる
    instance.healthBorder = { attack3 = 0.5 }
    -- 攻撃アクションによるSP増加量
    ...
    instance.transitionAttack = function (this, unit, index)
        local item = nil
        -- HPが50%以下ならランダム攻撃にattack3が加わる
        if this:isDegradedHealth(unit, this.keyNames.attack3) then
            item = this:randomPickItem(this.attackProbs)
        else
            -- それ以外ならattack1とattack2のみの抽選を行う
            local items = this:findItemsFromKey(this.attackProbs, this.keyNames.attack1, this.keyNames.attack2)
            item = this:randomPickItem(items)
        end
