...
local class = summoner.Bootstrap.createUnitClass({label="シュリ", version=1.8, id=102855512});
...
function class:setSurfaceBoxList()
   ...
   self.SURFACE_BOX_LIST = {
      ...
      [self.afterMD] = {
         ACTIVE_SKILL = 3,
         VOICE = "VOICE_FULLARTS_CUTIN_B",
         SKILL_NAME = self.TEXT.AFTER_MD or "真・刀舞「星火燎原」",
...
function class:takeDamageValue(event)
  if self.isAura then
    local baseValue = (event.value/event.unit:getCalcHPMAX()) * 100
    event.unit:addSP(baseValue * 2)
  end
  return event.value
end
...
function class:run(event)
   ...
   if event.spineEvent == "loopStart" then
      ...
      if self:findWeapon(event.unit,20182700) then
         event.unit:addSP(50)
      end
   end
   return 1
...
function class:takeSkill(event)
   ...
   if event.index == 3 then
      if not self.isAura then
         ...
         event.unit:setNextAnimationEffectName("2-skill3_2")
