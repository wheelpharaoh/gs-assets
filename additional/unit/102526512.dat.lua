...
local class = summoner.Bootstrap.createUnitClass({label="デューク", version=1.3, id=102526512});
   ...
   if event.spineEvent == "addBreak" then
      if megast.Battle:getInstance():isHost() then
         for i = 0,7 do
             ...
             if value ~= nil then
                megast.Battle:getInstance():sendEventToLua(self.scriptID,1,value * 10 + i);
             end
         end
...
function class:setBreakValue(index,value)
   ...
   self.enemyList[index] = breakValue
   ...
   for i = 0,7 do
       ...
       if value ~= nil then
          ...
          if localUnit ~= nil then
            value = value * self:getBuffValue(localUnit,27)
...
-- function class:findBreakTarget()
--    for i = 0,7 do
--       local value = self.enemyList[i]
--       if value ~= nil then
--          self:setBreakTarget(i,value)
--       end
--    end
-- end

-- function class:setBreakTarget(index,value)
--     local localUnit = megast.Battle:getInstance():getTeam(not self.gameUnit:getisPlayer()):getTeamUnit(index)
--     if localUnit ~= nil then
--        self:execAddBreak(localUnit,value)
--     end
-- end

-- function class:execAddBreak(unit,value)
--      local breakValue = value * self:getBuffValue(unit,27)
--      unit:setBreakPoint(unit:getBreakPoint() - breakValue)
--      if megast.Battle:getInstance():isRaid() then
--         RaidControl:get():addBreakPool(breakValue); 
--      end
-- end


--耐性のときだけ取りたい
function class:getBuffValue(unit,id)
   local buff = unit:getTeamUnitCondition():findConditionWithType(id)
   local value = 1
   if buff ~= nil and buff:getValue() < 0 then
     value = 1 + (buff:getValue() * 0.01)
   end
   if value < 0 then
      value = 0
   end
