...
local class = summoner.Bootstrap.createUnitClass({label="サク", version=1.8, id=102886312});
...
function class:setSurfaceBoxList()
   ...
   self.SURFACE_BOX_LIST = {
      ...
      [self.afterMD] = {
         ...
         SKILL_NAME = self.TEXT.AFTER_MD or "我式連術「曼珠沙華」",
...
function class:attackDamageValue(event)
    ...
    if type == 1 then
       local orbit = event.enemy:addOrbitSystemWithFile("10200ef","empty");
       self:setOrbitPosition(event.enemy,orbit)
       orbit:setAnimation(0,"skill1_hit",false)
    end 
...
function class:setAura(unit)
   ...
   self.aura = unit:addOrbitSystemWithFile("Kaigan3","Kaigan_in");
   self.aura:takeAnimation(0,"Kaigan_in",true);
