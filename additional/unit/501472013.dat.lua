...
local class = summoner.Bootstrap.createUnitClass({label="カボチャ無", version=1.3, id=501472013});


class.UNIT_ID = 501472013;
class.HP_RATE = 0.008;
class.POINT = 666666;
...
function class:start(event)
	...
	self.TEXT.DEAD_MESSAGE1 = self.TEXT.DEAD_MESSAGE1 or "ボーンパンプキンを撃破して+%dpt";
...
function class:dead(event)
	...
	summoner.Utility.messageByEnemy(message,5,white);
...
function class:sarchConditionTartget(unit,deltaTime)
	...
	local team = megast.Battle:getInstance():getTeam(true);
	for i=0,7 do
        local target = team:getTeamUnit(i);
        if target ~= nil and target:getTeamUnitCondition():findConditionWithID(self.BUFF_LIST[1].ID) == nil then
            self:execAddBuff(target,self.BUFF_LIST[1]);
			self:execAddBuff(target,self.BUFF_LIST[2]);
...
function class:removeConditionTartget(unit)
	...
	for i=0,7 do
        ...
        if target ~= nil then
            ...
            local buff2 = target:getTeamUnitCondition():findConditionWithID(self.BUFF_LIST[2].ID);
	    	if buff2 ~= nil then
	        	target:getTeamUnitCondition():removeCondition(buff2);
	        end
        end
    end
