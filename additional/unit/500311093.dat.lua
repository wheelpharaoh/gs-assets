function new(id)
    ...
    local instance = {
        ...
        target = function(this,unit)
            local tmp = {};
            for i = 0,4 do
                local tgt = megast.Battle:getInstance():getTeam(not this.isPlayerUnit):getTeamUnit(i);
                if tgt ~= nil then
                    table.insert(tmp,tgt);
                end 
            end

            local rand = LuaUtilities.rand(0,table.maxn(tmp));
            unit:setTargetUnit(megast.Battle:getInstance():getTeam(not this.isPlayerUnit):getTeamUnit(rand));
            return 1;
        end,
        ...
        excuteAction = function (this , unit)
            ...
            if not this.isChangedRange and this.beforTarget ~= nil then
                ...
                if rand < 30 and this.isInfight then
                    print("maxrange=10");
                    ...
                    if not this.isInfight then
                        unit:setRange_Min(200);
                    end
                end
