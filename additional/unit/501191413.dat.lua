local class = summoner.Bootstrap.createUnitClass({label="まーる", version=1.6, id=501191413});
...
function class:endless(unit)
    ...
    if megast.Battle:getInstance():isHost() then
        megast.Battle:getInstance():sendEventToLua(self.scriptID,2,self.state);
    end
end
...
function class:excuteTrigger(unit,trigger)
    if trigger == "modeChange" then
        ...
        if not self.getIsHost() then
            megast.Battle:getInstance():sendEventToLua(self.scriptID,3,unit:getHP());
        end
    end
end
...
function class:receive2(args)
    if not self.isEndless then
        self.gameUnit:takeSkill(4);
    end
    return 1;
end

function class:receive3(args)
    self.gameUnit:setHP(args.arg);
    return 1;
end
