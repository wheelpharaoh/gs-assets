...
local class = summoner.Bootstrap.createUnitClass({label="パルラミシア", version=1.3, id=102316212});
...
function class:attackDamageValue(event)
  ...
  if self.isCounterSuccess and self.isTwice then
    self.isDone = true
    self:execRemoveCondition(event.unit,self.BUFF_BOX_LIST[0].BUFF_ID)
