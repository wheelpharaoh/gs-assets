    ...
    local instance = {
        ...
        addbuff = function (this,unit)
                for i = 0,3 do
                    ...
                    if teamUnit ~= nil then
                       local old = teamUnit:getTeamUnitCondition():findConditionWithGroupID(2000);
                       if old ~= nil then
                           teamUnit:getTeamUnitCondition():removeCondition(old);
                       end
                       local cond = teamUnit:getTeamUnitCondition():addCondition(2000, 98, 1000, 7, 24,1);
                       cond:setGroupID(2000);
                       teamUnit:playSummary(summoner.Text:fetchByUnitID(101205412).text1,true);
                    end
                end
            return value;
        end,
