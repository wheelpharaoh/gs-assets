...
-- Funnelクラスを定義
    ...
    instance.Funnel.new = function (funnelUnit, ownerUnit)
        ...
        instance.orbits = {
            attack1 = "attack1spirit", attack1ball = "attack1spiritBall", attack1hit = "attack1spiritExplosion", skill1 = "skill1spirit", skill2 = "skill2spirit", skill3 = "skill3spirit"
        }
    ...
    instance.isControllTarget = function(this,unit)
    ...
    instance.checkSkill = function (this,unit,index)
        if not this.isControllTarget(this,unit) then
            return;
        end

        if this.aula ~= nil then
            return;
        end
        ...
        if index == 2 then 
            ...
            this.iconControll(this,unit,index);
            megast.Battle:getInstance():sendEventToLua(this.uniqueID,2,index);
        end
        if index == 3 and this.isSkillUsed then
            this.isSkillUsed = false;
            this.takeAula(this,unit);
            this.iconControll(this,unit,index);
            megast.Battle:getInstance():sendEventToLua(this.uniqueID,2,index);
            megast.Battle:getInstance():sendEventToLua(this.uniqueID,1,0);
        end
    end

    instance.iconControll = function(this,unit,index)
		if index == 2 then 
          
            unit:getTeamUnitCondition():addCondition(100610,0,1,99999,163);
        end
        if index == 3 then 
            local buff = unit:getTeamUnitCondition():findConditionWithID(100610);
            if buff ~= nil then
    ...
    instance.takeAula = function(this,unit)
        this.aula = unit:addOrbitSystem("buffOn");
        this.aula:takeAnimation(0,"buffOn",true);
        unit:takeAnimation(0,"buffOn",false)
        this.aula:setPosition(unit:getAnimationPositionX(),unit:getAnimationPositionY() - 60);
        this.aulaTimer = 0;
        unit:getTeamUnitCondition():addCondition(100611,28,30,30,7);
        unit:getTeamUnitCondition():addCondition(100612,29,100,30,34);
        unit:getTeamUnitCondition():addCondition(100613,30,100,30,33);
        unit:getTeamUnitCondition():addCondition(100614,17,100,30,26);
        unit:getTeamUnitCondition():addCondition(100615,25,100,30,9);
        unit:playSummary(summoner.Text:fetchByUnitID(100616112).mess1 , true);
    end

    instance.buffLoop = function(this,unit)
        this.aula:takeAnimation(0,"buffLoop",true);
    end

    instance.buffEnd = function(this,unit)
        this.aula:takeAnimation(0,"buffOff",false)
        unit:takeAnimation(0,"buffOff",false)
        this.downTimer = 0;
        this.isDown = true;
        this.aula = nil;
        unit:getTeamUnitCondition():addCondition(100616,89,1,12,79);
    end
    ...
    instance.receive1 = function (this , intparam)
        this.takeAula(this,this.gameUnit);
        return 1;
    end

    instance.receive2 = function (this , intparam)
        this.iconControll(this,this.gameUnit,intparam);
        return 1;
    end
    ...
    instance.run = function (this , unit , str)
        ...
        if str == "checkSkill2" then
            this.checkSkill(this,unit,2);

        end

        if str == "checkSkill3" then
            this.checkSkill(this,unit,3);
        end

        if str == "buffLoop" then
            this.buffLoop(this,unit);
        end
    ...
    instance.update = function (this , unit , deltatime)
        ...
        if this.aula ~= nil then
            this.aula:setPosition(unit:getAnimationPositionX(),unit:getAnimationPositionY()-60);
            this.aulaTimer = this.aulaTimer + deltatime;
            
            if this.aulaTimer > 30 and unit:getBurstState() ~= kBurstState_active then
                this.buffEnd(this,unit);
            end
        end

        if this.isDown then
            this.downTimer = this.downTimer + deltatime;
            if this.downTimer > 12 then
                this.isDown = false;
            end
        end
    ...
    instance.takeDamage = function (this , unit)
        ...
        if this.isDown then
            unit:setNextAnimationName("down");
        end
        return 1;
    end
