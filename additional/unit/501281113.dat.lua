...
local class = summoner.Bootstrap.createUnitClass({label="501311413", version=1.3, id=501281113});
...
function class:start(event)
    ...
    self.START_MESSAGES = {
        [0] = {
            MESSAGE = self.TEXT.START_MESSAGE or "炎属性ユニットからのダメージ・ブレイク無効",
            COLOR = Color.red,
            DURATION = 15
        }
    }
    ...
    self.BUFF_MESSAGES1 = {
        [0] = {
            MESSAGE = self.TEXT.BUFF_MESSAGE1 or "防御力無視",
            COLOR = Color.red,
            DURATION = 15
        }
    }
    ...
    self.BUFF_MESSAGES2 = {
        [0] = {
            MESSAGE = self.TEXT.BUFF_MESSAGE2 or "HP自然回復",
            COLOR = Color.red,
            DURATION = 15
        }
    }
    ...
    self.BUFF_MESSAGES3 = {
        [0] = {
            MESSAGE = self.TEXT.BUFF_MESSAGE3 or "ダメージアップ",
            COLOR = Color.red,
            DURATION = 15
        }
    }
...
function class:update(event)
    ...
    self.lightUpCounter = self.lightUpCounter + event.deltaTime;
    if self.lightUpCounter > self.lightUpTime then
        self.lightUpCounter = self.lightUpCounter - self.lightUpTime;
        self:showLightUp(event.unit);
    end
    return 1;
end
...
function class:showLightUp(unit)
...
function class:excuteTrigger(unit,trigger)
    if trigger == "addBuff1" then
        if self.crystalBuffs[1] ~= nil then
            self.crystalBuffs[1].action(self,unit);
            self.crystalBuffs[1] = nil;
            megast.Battle:getInstance():sendEventToLua(self.scriptID,6,1);
        end
    end

    if trigger == "addBuff2" then
        if self.crystalBuffs[2] ~= nil then
            self.crystalBuffs[2].action(self,unit);
            self.crystalBuffs[2] = nil;
            megast.Battle:getInstance():sendEventToLua(self.scriptID,6,2);

            self.crystalBuffs[3].action(self,unit);
            self.crystalBuffs[3] = nil;
            megast.Battle:getInstance():sendEventToLua(self.scriptID,6,3);
        end
    end

    -- if trigger == "addBuff3" then
    --     if self.crystalBuffs[3] ~= nil then
    --         self.crystalBuffs[3].action(self,unit);
    --         self.crystalBuffs[3] = nil;
    --         megast.Battle:getInstance():sendEventToLua(self.scriptID,6,3);
    --     end
...
function class:receive3(args)
    if self.crystalBuffs[args.arg] ~= nil then
        self.crystalBuffs[args.arg].action(self,unit);
        self.crystalBuffs[args.arg] = nil;
    end
    return 1;
end
...
function class:receive6(args)
    self.crystalBuffs[args.arg].action(self,self.gameUnit);
    self.crystalBuffs[args.arg] = nil;
    return 1;
end
